...
# parameter tune
python3 ../sarsalambda.py --num-episodes $1 --gamma $2 --lambda $3 --epsilon $4 --alpha $5 --seed $6 --data_path ${13} --layout ${14} --feature ${15}
                ...
                for seed in "${SEED[@]}"; do
                    for layout in "${LAYOUT[@]}"; do
                        for feature in "${FEATURE[@]}"; do
                            python3 ../sarsalambda.py --num-episodes $1 --gamma $gamma --lambda $lambda --epsilon $epsilon --alpha $alpha --seed $seed --data_path ${13} --layout $layout --feature $feature
                        done
                    done
                done
            done
